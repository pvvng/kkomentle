...
### [꼬들꼬들](https://kkodle-kkodle.netlify.app/) 👈 클릭
...
## 6. 후기
### 느낀 점
- 전에 했던 토이프로젝트가 전부 일주일 만에 끝냈더라고요. 그래서 디테일적인 부분이 부족하다고 느꼈는데 이번 프로젝트는 시간 여유롭게 잡고 천천히 해봤습니다. 아직 추가할게 많지만 나름 나쁘지 않다고 생각합니다.
- zustand 너무 좋습니다. 프로젝트 규모가 커지면 문법 빡빡한 redux가 더 좋을 것 같긴한데 소규모 프로젝트에는 zustand가 최고인듯요.
- open AI API로 할 수 있는게 임베딩 말고도 다양하게 많더라구요. 다음엔 가상 여친을 만들어보고 싶습니다. 장난이고 추리 게임 같은거 만들어도 재밌을 것 같습니다.
- tailwind 특유의 코드 덕지덕지 붙히는 느낌이 싫어서 bootstrap 사용했는데 어느 순간 코드 덕지덕지 붙히는 저를 발견했습니다. 다음 프로젝트엔 tailwind 써볼까 고민중입니다.
- 꼬들꼬들 많이 해주세요. 열심히 만들었는데 나밖에 안함.
