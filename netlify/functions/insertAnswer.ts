...
import getEmbeddingForCron from "./getEmbedding";

/** db에 오늘의 단어 추가 하는 함수 */
export default async function insertAnswerForCron(todayAnswer :string, type :string) {
  ...
  if(!findDateResult){
    try {
      let embedding = await getEmbeddingForCron(todayAnswer);
